...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 订阅音频列表管理 <span class="c-gray en">&gt;</span> 订阅音频管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>

<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20">
	<?php $id=$_GET['id']; ?>
	<span class="l"><a  style="text-decoration:none href="javascript:;" onclick="this,del_()"  class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a></span><span class="r">共有数据：<strong><?php   $id = $_GET['id'];require("../php/config.php"); $res=mysql_query("select count(*) as total from `shi-subscribe-au` where customerid = $id"); if($row=mysql_fetch_array($res)){ $num=$row['total'];}  echo $num;?></strong> 条</span> </div>
	...
	<?php
    	...
    	JXMySQL_Execute("SELECT au.id,title,a.id as a from  `shi-subscribe-au` as au 
						LEFT JOIN  `shi-audio` as a 
						ON au.audioid= a.id 
						LEFT JOIN `shi-custmoer` as c 
						ON au.customerid=c.id
...
function article_del(obj,id){
	...
	layer.confirm('确认要删除吗？',function(index){
		...
		$.ajax({
			type: 'GET',
			url: '../php/subscribeaudio_del.php',
			dataType: 'json',
			data   : {Id: id},
			success: function(data){
 ...
 function del_(obj,id) {
  ...
  if (ids.length == 0) {
    ...
    if (layer.confirm('确认要删除吗？',function(index){
      ...
      $.ajax({
        type: "GET",
        url: "../php/subscribeaudio_dels.php",
        data: url,
        dataType:"json",
       success: function(data){
