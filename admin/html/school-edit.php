...
<article class="page-container">
    ...
    <form class="form form-horizontal" id="form-article-add" >
        ...
        <?php 
                ...
                JXMySQL_Execute("select * from `shi-arschool` where id = ?",array($id));
                $list=JXMySQL_Result();
                foreach ($list as $k => $v) {
                 ?>
    ...
    <div class="row cl">
        <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>流派名：</label>
        <div class="formControls col-xs-8 col-sm-9" style="width:400px;">
            <input  type="text" class="input-text" value="<?php echo $v['Schools'] ?>" placeholder="" id="adminName" name="adminName" onfocus="get()" onblur="check()" style="width:200px"><input type="text" id="check1" style="display:none;float:right;">
        </div>
    </div>
...
function tijiao(){
    ...
    $.ajax({
        ...
        success:function (data){
            if (data.Code== 0) {
                window.location.href = "./school-list.php";
            }else{
                alert('修改失败');
            }
        }
