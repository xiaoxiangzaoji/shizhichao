...
if($img['error']>0){
	$sql=JXMySQL_Execute("insert into `shi-reader` (teacher,headimg) values(?,?)",array($nickname,''));
...
header("Location:../html/teacher-list.php");
	...
	if(strtolower($type)=='.png'||strtolower($type)=='.jpg'||strtolower($type)=='.bmp'||strtolower($type)=='.gif'||strtolower($type)=='.jpeg')//判断上传的文件是否为图片格式
	{
		...
		$sql=JXMySQL_Execute("insert into `shi-reader` (teacher,headimg) values(?,?)",array($nickname,$headimg));
	    $insert_id = JXMySQL_Insert($sql);
	    if ($insert_id) {
	        header("Location:../html/reader-list.php"); 
	        exit;
