...
if (empty($_FILES['headimg']['size'])) {
	...
	$sql=JXMySQL_Execute("update `shi-readers` set teachername = ?,info = ?,stars=?,cate=? where id = ?",array($teachername,$info,$stars,$cate,$id));
	...
	if(isset($img))
	{
		...
		if(strtolower($type)=='.png'||strtolower($type)=='.jpg'||strtolower($type)=='.bmp'||strtolower($type)=='.gif'||strtolower($type)=='.jpeg')//判断上传的文件是否为图片格式
		{
			...
			$url=$path;
			$sql=JXMySQL_Execute("update `shi-readers` set teachername = ?,info = ?,stars=?,headimg= ?,cate=? where id = ?",array($teachername,$info,$stars,$url,$cate,$id));
